...
(function () {'use strict';
  for (var
    ...
    parse = isNodeJS ?
      ...
      (function () {
        return function (txt) {
          ...
          match(txt, 'synFunc', out);
          match(txt, 'synVar', out);
          match(txt, 'synClass', out);
          match(txt, 'synStr', out);
          match(txt, 'synNum', out);
          ...
          txt = replace(txt, 'synFunc');
          txt = replace(txt, 'synVar');
          txt = replace(txt, 'synClass');
          txt = replace(txt, 'synStr');
          txt = replace(txt, 'synNum');
          //txt = txt.replace(singleLineCode, storeAndHide);
    ...
    transform = {
      ...
      synFunc: {
        re: /(<(?:func|meth)>)(\()(\w+)(\(\))?(\))/g,
        place: function ($0, $1, $2, txt, $4) {
          return '%c' + txt + ($4 ? $4 : "") + '%c';
        },
        start: 'color:#4DA586;font-family:monospace;',
        end: 'color:initial;font-family:default;'
      },
      synVar: {
        re: /(<(?:var|prop|attr)>)(\()(\w+)?(\))/g,
        place: function ($0, $1, $2, txt) {
          return '%c' + txt + '%c';
        },
        start: 'color:#D9C679;font-family:monospace;',
        end: 'color:initial;font-family:default;'
      },
      synClass: {
        re: /(<(?:class)>)(\()(\w+)?(\))/g,
        ...
        start: 'color:#D88E79;font-family:monospace;',
        end: 'color:initial;font-family:default;'
      },
      synStr: {
        re: /(<(?:str|string)>)(\()(\w+)?(\))/g,
        ...
        start: 'color:#60C0D2;font-family:monospace;',
        end: 'color:initial;font-family:default;'
      },
      synNum: {
        re: /(<(?:num|number|float|int)>)(\()(\w+)?(\))/g,
        place: function ($0, $1, $2, txt) {
          return '%c' + txt + '%c';
        },
        start: 'color:#609AD2;font-family:monospace;',
        end: 'color:initial;font-family:default;'
      },
      dim: {
        re: /(-{1,2})(?=\S)(.*?)(\S)\1/g,
    ...
    overwrite = function (method) {
      ...
      if (original) (consolemd[method] = isNodeJS ?
        ...
        function () {
          ...
          if (otherArgs.length > 1) otherArgs.splice(0, 1);
          //return original.apply(console, [otherArgs]);
          return arguments.length === 1 && typeof arguments[0] === 'string' ?
          original.apply(console, parse(arguments[0])) :
          typeof arguments[0] === 'string' ?
          original.apply(console, parse(arguments[0]).concat(otherArgs)) :
